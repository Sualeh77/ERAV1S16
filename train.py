...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = WordLevelTrainer(
            special_tokens=["[UNK]", "[PAD]", "[SOS]", "[EOS]"], min_frequency=4
        )
...
def get_ds(config):
    ...
    # For filtering out tokens more than 150
    ...
    filtered_sorted_train_ds = [k for k in sorted_train_ds if len(k["translation"][config["lang_src"]]) + 10 > len(k["translation"][config["lang_src"]])]
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=lambda batch: collate_fn(batch, tokenizer_tgt),
    )
...
def train_model(config, scheduler):
    # Define the device
    ...
    # if the user specifieed a model to preload before training, load it
    initial_epoch = 0
    global_step = 0
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            encoder_input = batch["encoder_input"].to(device)  # (b, seq_len)
            decoder_input = batch["decoder_input"].to(device)  # (B. sea len)
            encoder_mask = batch["encoder_mask"].to(device)  # (B, 1, 1, seq_len)
            decoder_mask = batch["decoder_mask"].to(device)  # (B, 1, seq_len, seq len)
